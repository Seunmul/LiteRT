...
// +----------------------------------------------+  |  |  |
        ...
        TfLiteDelegateParams **partition_params_array, int *num_partitions)
    {
        ...
        // Defaults.
        if (!partition_params_array || !num_partitions)
            return kTfLiteError;
        ...
        const TfLiteNode *node, const TfLiteRegistration *registration) const
    {
        ...
        if (AnyTensorOfTypeResource(tensors_, node->inputs))
            return true;
        // Check if any of the output tensors are of type resource.
        if (AnyTensorOfTypeResource(tensors_, node->outputs))
            return true;
    ...
    TfLiteStatus Subgraph::ReleaseMemory()
    {
        ...
        {
            if (input_tensor_idx == kTfLiteOptionalTensor)
                continue;
        ...
        {
            if (output_tensor_idx == kTfLiteOptionalTensor)
                continue;
            ...
            if (op_reg.registration_external->node_index != -1)
            {
                TfLiteRegistration *referenced_registration =
                    &nodes_and_registration_[op_reg.registration_external->node_index]
                         .second;
                if (referenced_registration->init == nullptr)
                    return nullptr;
        ...
        if (op_reg.registration_external)
        {
            if (op_reg.registration_external->node_index != -1)
            {
                TfLiteRegistration *referenced_registration =
                    &nodes_and_registration_[op_reg.registration_external->node_index]
                         .second;
                if (referenced_registration->invoke == nullptr)
                    return kTfLiteError;
        ...
        if (op_reg.registration_external && buffer)
        {
            if (op_reg.registration_external->node_index != -1)
            {
                TfLiteRegistration *referenced_registration =
                    &nodes_and_registration_[op_reg.registration_external->node_index]
                         .second;
                if (referenced_registration->free == nullptr)
                    return;
    ...
    TfLiteStatus Subgraph::InvokeImpl()
    {
             ...
             execution_plan_index < execution_plan_.size(); execution_plan_index++)
        {
            ...
            //           << "Invoking operator(op_index): " << op_name <<"("<< registration.builtin_code << ")"
                                      ...
                                      int *first_new_tensor_index)
    {
        ...
        if (first_new_tensor_index)
            *first_new_tensor_index = base_index;
        if (tensors_to_add < 0)
            return kTfLiteError;
        ...
        const size_t buffer_identifier)
    {
            ...
            EqualArrayAndTfLiteIntArray(tensor.dims, ndims, dims))
        {
            ...
            if (!tensor.dims)
                tensor.dims = ConvertArrayToTfLiteIntArray(ndims, dims);
    ...
    TfLiteStatus Subgraph::UndoAllDelegates()
    {
             ...
             execution_plan_index < execution_plan_.size(); ++execution_plan_index)
        {
            ...
            for (int i = 0; i < node.inputs->size; ++i)
            {
                ...
                if (original_input_idx == kTfLiteOptionalTensor)
                    continue;
    ...
    void Subgraph::MaybeReleaseDynamicTensors(const TfLiteNode &node,
                                              size_t node_index)
    {
        if (!ShouldReleaseDynamicTensors())
            return;
